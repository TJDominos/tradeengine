...
import {
  StrategyEngineDurableObject,
  strategyEngineDurableObjectNameFor,
  type StrategyEngineDurableObjectConfigureRequest,
  type StrategyEngineDurableObjectEventRequest,
} from './backend/strategy/strategyEngineDO';
import {
...
import { StrategyEngine } from './backend/strategy/engine';
import {
...
type StrategyRuntimeRegistryEntry = {
  userId: number;
  versionId: number;
...
const strategyRuntimeRegistry = new Map<string, StrategyRuntimeRegistryEntry>();
...
function buildStrategyTaskExecutionContext(
  env: Env,
  userId: number,
...
async function handleStrategyExternalTradeWebhook(
  request: Request,
  url: URL,
  env: Env,
...
async function handleApi(
  ...
  try {
    ...
    if (method === 'POST' && pathname === '/api/webhooks/strategy/external-trade')
      return await handleStrategyExternalTradeWebhook(request, url, env, ctx);
...
export { StrategyEngineDurableObject };
