...
const persistDir = mkdtempSync(path.join(tmpdir(), 'tradeengine-do-idempotency-'));
...
try {
  ...
  const bootstrap = await requestJson('/api/auth/bootstrap', {
    method: 'POST',
    body: JSON.stringify({ username: 'ci-admin-idem', password: 'ci-password-123' }),
  });
  ...
  await waitFor(
    async () => {
      ...
      if (payload.active?.versionId === queueVersionId && payload.currentMetrics) {
        return payload;
      }
      return null;
    },
    {
      timeoutMs: 30000,
      intervalMs: 500,
      timeoutMessage: 'Timed out waiting for idempotency test strategy to become active',
    },
  );

  const txHash = `ci-idempotency-${Date.now()}`;
  ...
  const firstWebhook = await requestJson('/api/webhook', {
    method: 'POST',
    body: JSON.stringify(webhookPayload),
  });
  assert.equal(firstWebhook.payload.forwarded, true, 'first webhook should be forwarded to the active DO');
  assert.equal(firstWebhook.payload.duplicate, false, 'first webhook must not be marked duplicate');

  const secondWebhook = await requestJson('/api/webhook', {
    method: 'POST',
    body: JSON.stringify(webhookPayload),
  });
  assert.equal(secondWebhook.payload.forwarded, true, 'duplicate webhook should still reach the DO endpoint');
  assert.equal(secondWebhook.payload.duplicate, true, 'second webhook with same txHash must be marked duplicate');

  const snapshotAfterDuplicate = await requestJson('/api/strategy/current', {
    method: 'GET',
    headers: authHeaders,
  });
  assert.equal(snapshotAfterDuplicate.payload.active?.versionId, queueVersionId, 'duplicate webhook should not disrupt the active strategy binding');
  assert.ok(
    !snapshotAfterDuplicate.payload.history?.some((entry) => entry.versionId === queueVersionId),
    'duplicate webhook should not force the strategy into history',
  );
